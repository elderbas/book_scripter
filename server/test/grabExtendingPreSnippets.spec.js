...
describe('grabExtendingPreSnippets', () => {
  ...
  describe('allExtended', () => {
    it(`returns the pre snippet selected + extending snippets`, function () {
      ...
      let indexSelected = 3;
      expect(grabExtendingPreSnippets(mockSnippetList, indexSelected).allExtended).to.deep.equal(output);
    });
    ...
    it(`custom # to grab up to on sides (left side falls short)`, function () {
      ...
      expect(grabExtendingPreSnippets(mockSnippetList, indexSelected, quantityToGrabOnSides).allExtended).to.deep.equal(output);
    });
